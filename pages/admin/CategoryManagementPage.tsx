...
export const CategoryManagementPage = () => {
  ...
  const fetch = async () => {
    ...
    try {
      ...
      // fetch all events once to compute per-category counts (used in the UI)
      try {
        const allEvents = await eventsApi.getEvents();
        const map: Record<number, number> = {};
        (allEvents || []).forEach((e: any) => { if (e.categoryId) { map[Number(e.categoryId)] = (map[Number(e.categoryId)] || 0) + 1; } });
        setEventsCount(map);
      } catch (e) {
