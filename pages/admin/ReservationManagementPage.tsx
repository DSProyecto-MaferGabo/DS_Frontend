            ...
            <div className="bg-base-200 shadow-md rounded-lg overflow-x-auto">
                <table className="min-w-full">
                    <thead className="bg-base-300">
                        <tr>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm">ID Reserva</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm">Evento</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm">Usuario</th>
                    ...
                    <tbody className="text-gray-300">
                        {reservaciones.map((res) => (
                            <tr key={res.id} className="border-b border-base-300 hover:bg-base-300/50">
                                ...
                                <td className="py-3 px-4">{res.usuario?.nombre || 'N/A'} ({res.usuario?.email})</td>
                                <td className="py-3 px-4 font-semibold">${res.total.toFixed(2)}</td>
                                <td className="py-3 px-4">{new Date(res.fecha).toLocaleDateString()}</td>
